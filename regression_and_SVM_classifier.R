...
alda_regression <- function(x_train, x_test, y_train, regression_type){
  if(regression_type == 'linear'){ 
    fit1=glmnet(x_train,y_train,lambda=0)
    pr<-predict(fit1,x_test)
    return(list(fit1,pr))
    
  }else if(regression_type == 'ridge'){
    fit <- cv.glmnet(x_train, y_train, alpha=0, type.measure='mse',nfolds=10)
    pr<-predict(fit,x_test,s = fit$lambda.min)
    return(list(fit,pr))
    
  }else{
    fit <- cv.glmnet(x_train, y_train, alpha=1, type.measure='mse',nfolds=10)
    pr<-predict(fit,x_test,s = fit$lambda.min)
    return(list(fit,pr))
